...
public class Day5t2{
    ...
    String[] sort_to_valid(List<String> rules, String[] sorted_list){
        Day5 day = new Day5();
        //System.err.println(Arrays.toString(sorted_list));
        for (int i = sorted_list.length-1; i > 0; i--) {
            String temp = sorted_list[i-1] + "|" +  sorted_list[i];
            if (day.test_valid_one(rules, temp) == false){
                String placeholder;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < input.size(); i++) {
            ...
            if (day.test_valid_all(rules, temp) == false){
                temp = day2.sort_list(temp);
                temp = day2.sort_to_valid(rules, temp);
